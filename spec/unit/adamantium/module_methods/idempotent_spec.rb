...
describe Adamantium::ModuleMethods, '#memoize' do
  subject { object.idempotent(method) }
  ...
  context 'method with nonzero arity' do
    ...
    it 'should raise error' do
      expect { subject }.to(raise_error(Adamantium::ArityError) do |exception|
        exception.message.should == "#single_argument has nonzero arity so cannot be idempotent/memoized"
      end)
    end
  end
  ...
  context 'public method' do
    ...
    it_should_behave_like 'idempotens method'
